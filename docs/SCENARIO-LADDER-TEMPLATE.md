# Scenario Ladder Template

Use this template to create a new scenario file for any Velocidad **study**.

Scenario files live in `{study}/scenarios/` and define a single real-world deployment context:
where you'll practice, what success looks like at each level, and what vocabulary/structure is
specifically needed for that context. (A scenario is what earlier versions of the Spanish study
called a "world" — now unified across all studies.)

Name files with a short, memorable slug: `mcdonalds.md`, `client-standup.md`, `socratic-dialogue.md`.
...
[Any study-specific or cultural context that affects how this scenario works. What are the
...
[Study- or scenario-specific rules for the agent beyond the base rules-of-immersion. Examples:]
...
See `spanish/scenarios/mcdonalds/scenario.md` in this repository for a complete reference example.
